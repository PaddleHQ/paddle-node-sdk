...
describe('CheckoutDomainsResource', () => {
  test('should filter and return checkout domains with verification in progress', async () => {
    ...
    expect(paddleInstance.get).toHaveBeenCalledWith('/checkout-domains?status=verification_in_progress');
    expect(checkoutDomains).toHaveLength(1);
    expect(checkoutDomains[0]?.status).toBe('verification_in_progress');
